...
```{r data_loads, message = FALSE}

bills <- read_tsv(here::here("data", "billboard_songs.txt"))
...
```{r, songs_w_features, message=FALSE}

full_art <- bills %>% pull(artist)
test <- str_subset(full_art, pattern = "featur")

bills %>%
  filter(artist %in% test) %>%
  mutate(feat = str_sub(artist, 
                        start = str_locate(artist, pattern = "feat(.*)")),
         feat = str_sub(feat, start = str_locate(feat, pattern = "feat(.*)"),
                        end = str_locate(feat, pattern = " "))
         )
...
bill_feat <- bills %>%
  filter(artist %in% test) %>%
  mutate(features = str_sub(artist, 
      start = str_locate(artist, pattern = "featur(.*)"),
      end = -1),
    features = str_remove(features, pattern = "featuring "),
    artist = str_remove(artist, pattern = "featuring(.*)")
    )
